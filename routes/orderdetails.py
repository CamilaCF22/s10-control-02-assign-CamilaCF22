...
@login_required
def update(id, orderId):
    currentOrder = Order.query.filter_by(id=id).first()
    form = OrderDetailUpdateForm(status=currentOrder.status)
    if form.validate_on_submit():
        quantity = form.quantity.data
        description = form.description.data
        unitCost = form.unitCost.data
        ...
        db.session.add(currentOrder)
        db.session.commit()
        return redirect(url_for("orderDetails.home", userId=userId))
    return render_template("orderDetails/update.html", form=form, userId=userId, task=currentTask)
...
def delete(id, orderId):
    if current_id == id:
        currentorderdetails = Task.query.filter_by(id=id).first()
        db.session.delete(currentorderdetails)
        db.session.commit()
    return redirect(url_for("orderdetails/home", id=id))